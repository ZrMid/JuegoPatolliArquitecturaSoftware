...
public class InternalFrmTablero extends javax.swing.JInternalFrame {
    ...
    public void pintarFichas() {
        ...
        for (int i = 0; i < objMainPanel.objFichas.size(); i++) {
            ...
            int Fx = 904 / 2 - tamCasillaW;
            int Fy = 600 / 2 - tamCasillaH;
    ...
    public void moverFicha(int x, int y) {
        ...
        int limySup = (600 / 2) - ((tamCasillaH + 2) * (casillasAspa + 1)) - 2;
        int limyInf = (600 / 2) + ((tamCasillaH + 2) * (casillasAspa + 1));
        int limxDer = (902 / 2) + ((tamCasillaW + 2) * (casillasAspa + 1));
        int limxIzq = (902 / 2) - ((tamCasillaW + 2) * (casillasAspa + 1));

        if (y > limySup && y < 600 / 2 && x < 902 / 2) {
            y -= (tamCasillaH + 2) * canas;
            double residuo = (limySup - y) / (tamCasillaH + 2.0);
            ...
            if (residuo > 0.5 && residuo < 1.5) {
                ...
                y = limySup + 4;
                x += tamCasillaW + 2;
                y += (tamCasillaH + 2) * (int) Math.floor(residuo);
            }

        } else if (x < limxDer && x > limxIzq && y > (600 / 2 - (tamCasillaH - 2) * 2)) {
                x += tamCasillaW + 2;
        } else if (y < limyInf) {
            y += (tamCasillaH + 2) * canas;
        }
